...
export const listPlatformUpdates = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }): Promise<PlatformUpdate[]> => {
    const { data, error } = await context.supabase
      ...
      .select("id, title, description, link_path, link_label, published_at")
...
export const createPlatformUpdate = createServerFn({ method: "POST" })
  .middleware([requireActiveProfile])
  .inputValidator((d: { title: string; description: string; linkPath?: string; linkLabel?: string; publishedAt?: string }) =>
    z.object({
