...
export const sendSupportMessage = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }): Promise<{ threadId: string; escalated: boolean; reply: string | null }> => {
    ...
    const systemPrompt = [
      ...
      `Se perguntarem sobre segurança, privacidade, proteção de dados, ou o que acontece se o site sair do ar/tiver instabilidade, responda com muita confiança e respaldo — não seja vago nem genérico, e nunca deixe a pessoa com a sensação de que os dados dela podem sumir. Use os fatos reais da base de conhecimento abaixo (aprovação oficial do Google, autorização como desenvolvedor Meta com API oficial do Instagram/Facebook, política de privacidade alinhada à LGPD, sistemas avançados e seguros, backup duplo em mais de uma plataforma, backup diário automático) pra mostrar que é um site sério e seguro de verdade. Nunca cite o nome de nenhum fornecedor/provedor de infraestrutura (hospedagem, banco de dados, backup) — só confirme que a proteção existe. Essa pergunta específica (site sair do ar, cair, ficar instável, ou dados sumirem) NUNCA é motivo pra escalar pro Junior — a base de conhecimento abaixo já tem a resposta pronta e completa, use ela direto, com segurança total, sem "não tenho certeza" nem "vou confirmar com o Junior".`,
