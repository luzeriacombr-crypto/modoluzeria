...
export function ClientBlockedItemsModal({ clientId, onClose }: { clientId: string; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 backdrop-blur-sm p-4" onClick={onClose}>
      <div
        className="w-full max-w-md max-h-[80vh] flex flex-col bg-[#1C1C1C] border border-white/10 rounded-2xl overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="flex items-start justify-between px-6 py-4 border-b border-white/[0.08] shrink-0">
          <div>
            <h3 className="text-base font-semibold text-white">Conteúdos travados</h3>
            <p className="text-[11px] text-white/40 mt-0.5">Todos os meses, com o motivo do bloqueio</p>
          </div>
        ...
        <div className="overflow-y-auto">
          {isLoading ? (
            <p className="text-white/40 text-sm px-6 py-8 text-center">Carregando…</p>
          ) : items.length === 0 ? (
            <p className="text-white/40 text-sm px-6 py-8 text-center">Nenhum conteúdo travado. 🎉</p>
          ) : items.map((item) => (
            <div key={item.id} className="px-6 py-3 border-b border-white/[0.05] last:border-b-0">
              <div className="flex items-start gap-2">
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-white/40 shrink-0">{CONTENT_TYPE_LABEL[item.type as keyof typeof CONTENT_TYPE_LABEL] ?? item.type}</span>
                  </div>
