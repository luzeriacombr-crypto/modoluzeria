...
function MyAffiliateCard() {
  ...
  return (
    <div className="bg-gradient-to-br from-white/[0.08] to-white/[0.03] border border-white/15 rounded-2xl p-8 backdrop-blur-sm">
      <h2 className="text-2xl font-black text-white">Programa de Afiliados</h2>
      ...
      {isLoading ? (
        ...
        <div className="space-y-4">
          <div>
            <p className="text-xs font-bold uppercase text-white/40 mb-1">Seu link de indicação</p>
            <div className="flex items-center gap-2">
              <span className="flex-1 px-3 py-2 bg-white/[0.08] border border-white/10 rounded-lg text-white text-sm truncate">
                {referralLink}
              </span>
              <button
                onClick={copyLink}
            ...
            {copied && <p className="text-xs text-[rgb(var(--lz-brand-rgb))] mt-1">Copiado!</p>}
          </div>
          <a
...
function AdminAffiliateOverview() {
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-bold text-white">Todas as indicações (todas as agências)</h3>
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        <div className="bg-[#161616] border border-white/[0.07] rounded-xl p-5">
          <div className="flex items-center justify-between mb-2">
            ...
            <Users size={16} className="text-[rgb(var(--lz-brand-rgb))]" />
          </div>
          <div className="text-2xl font-black text-white">{list.length}</div>
        </div>
        <div className="bg-[#161616] border border-white/[0.07] rounded-xl p-5">
          <div className="flex items-center justify-between mb-2">
            <span className="text-white/60 text-sm font-semibold">Comissão pendente</span>
        ...
        <div className="bg-[#161616] border border-white/[0.07] rounded-xl p-5">
          ...
          <div className="text-2xl font-black text-white">R$ {totalCommission.toFixed(2)}</div>
        </div>
      </div>

      {list.length === 0 ? (
        <div className="text-center py-12 px-6 bg-white/[0.03] border border-white/10 rounded-2xl">
          <p className="text-white/50 text-sm">Nenhuma indicação ainda.</p>
        </div>
      ) : (
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-white/[0.07]">
                <th className="text-left px-4 py-3 text-xs font-semibold text-white/60">Afiliado</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-white/60">Agência</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-white/60">Indicado</th>
                <th className="text-right px-4 py-3 text-xs font-semibold text-white/60">Comissão</th>
                <th className="text-center px-4 py-3 text-xs font-semibold text-white/60">Status</th>
            ...
            <tbody>
              {list.map((r: any) => (
                <tr key={r.id} className="border-b border-white/[0.04] hover:bg-white/[0.02] transition">
                  <td className="px-4 py-3 text-sm text-white">{r.affiliate?.user?.name || "—"}</td>
                  <td className="px-4 py-3 text-sm text-white/60">{r.affiliate?.org?.name || "—"}</td>
                  <td className="px-4 py-3 text-sm text-white/60">{r.referred_user?.name || r.referred_user?.email || "—"}</td>
                  ...
                  <td className="px-4 py-3 text-center">
                    {!r.commission_paid && (
                      <button
                        ...
                        className="inline-flex items-center gap-1 px-3 py-1.5 text-xs font-bold text-white/70 hover:text-green-400 hover:bg-green-500/10 rounded-lg transition disabled:opacity-50"
