    ...
    <div className="grid grid-cols-[200px_1fr] gap-6">
      ...
      <div className="min-w-0">
        <div className="flex items-center gap-2 mb-4">
          <div className="flex items-center gap-2 flex-1 bg-card border border-foreground/10 rounded-md px-3 py-2 max-w-xs">
            <Search size={13} className="text-foreground/40 shrink-0" />
            <input
              value={search} onChange={(e) => setSearch(e.target.value)}
              placeholder="Buscar no fórum..."
              className="bg-transparent text-xs flex-1 outline-none placeholder:text-foreground/30 text-foreground"
            />
          </div>
          <button
        ...
        {isLoading ? (
          <div className="text-center py-16 text-foreground/40 text-sm">Carregando…</div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-16 px-6 bg-foreground/[0.03] border border-foreground/10 rounded-2xl">
            <MessageCircle size={22} className="mx-auto text-foreground/20 mb-2" />
            <p className="text-foreground/50 text-sm">{search ? "Nenhum post encontrado." : "Nenhum post ainda — seja o primeiro."}</p>
          </div>
        ) : (
  ...
  return (
    <button onClick={onClick}
      ...
      style={{
        ...
        color: active ? "#FFFFFF" : "color-mix(in srgb, var(--foreground) 70%, transparent)",
      }}>
      ...
      {count !== undefined && <span className="text-[10px] text-foreground/30 font-mono">{count}</span>}
    </button>
  );
}
...
function PostCard({ post, category, onClick }: { post: ForumPost; category?: ForumCategory; onClick: () => void }) {
  return (
    <button onClick={onClick} className="w-full text-left bg-card border border-foreground/7 rounded-xl p-4 hover:border-foreground/15 hover:bg-card transition-colors">
      <div className="flex items-center gap-2 mb-2">
        ...
        <span className="ml-auto text-[10.5px] font-mono text-foreground/30">{relTime(post.createdAt)}</span>
      </div>
      <h3 className="text-[15px] font-semibold text-foreground mb-1">{post.title}</h3>
      <p className="text-[13px] text-foreground/60 leading-relaxed mb-2.5" style={{ display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
        {post.body}
      </p>
      <div className="flex items-center gap-2">
        <Avatar name={post.authorName} size={20} />
        <span className="text-xs text-foreground font-medium">{post.authorName}</span>
        <span className="text-xs text-foreground/30">· {post.orgName}</span>
        <span className="ml-auto text-xs text-foreground/40 flex items-center gap-1">
  ...
  return (
    <>
      ...
      <div className="fixed z-50 bg-background border-foreground/10 flex flex-col
        ...
        <div className="px-5 pt-5 pb-4 border-b border-foreground/8 flex items-start gap-3">
          {cat && (
          ...
          {isPlatformAdmin && data && (
            <div className="ml-auto flex items-center gap-1">
              <button onClick={() => moderate(data.post.pinned ? "unpin" : "pin")} title={data.post.pinned ? "Desafixar" : "Fixar"}
                className="p-1.5 rounded text-foreground/40 hover:text-[var(--lz-accent-ink)] hover:bg-foreground/5">
                <Pin size={14} className={data.post.pinned ? "fill-current" : ""} />
              </button>
              <button onClick={() => moderate("delete")} title="Apagar post"
                className="p-1.5 rounded text-foreground/40 hover:text-red-400 hover:bg-foreground/5">
                <Trash2 size={14} />
              </button>
            </div>
          )}
          <button onClick={onClose} className="p-1.5 rounded text-foreground/40 hover:text-foreground hover:bg-foreground/5">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {isLoading || !data ? (
            <div className="text-center py-10 text-foreground/40 text-sm">Carregando…</div>
          ) : (
            <>
              <h2 className="text-lg font-semibold text-foreground leading-tight mb-3">{data.post.title}</h2>
              <div className="flex items-center gap-2.5 mb-4">
                <Avatar name={data.post.authorName} size={30} />
                <div>
                  <div className="text-sm font-semibold text-foreground">{data.post.authorName}</div>
                  <div className="text-[11.5px] text-foreground/40">{data.post.orgName} · {relTime(data.post.createdAt)}</div>
                </div>
              </div>
              <p className="text-[13.5px] text-foreground/70 leading-relaxed whitespace-pre-line">{data.post.body}</p>
              {data.post.linkUrl && (
                <a href={data.post.linkUrl} target="_blank" rel="noreferrer"
                  className="mt-3 flex items-center gap-2 bg-card border border-foreground/10 rounded-md px-3 py-2 text-xs text-[var(--lz-accent-ink)] hover:underline">
                  <ExternalLink size={13} /> {data.post.linkUrl}
                </a>
              )}

              <div className="text-[10.5px] font-bold uppercase tracking-wide text-foreground/30 mt-6 mb-2">
                {data.replies.length} resposta{data.replies.length === 1 ? "" : "s"}
              </div>
              {data.replies.map((r: ForumReply) => (
                <div key={r.id} className="flex gap-2.5 py-2.5 border-t border-foreground/6">
        ...
        <div className="border-t border-foreground/8 px-5 py-3 flex gap-2 items-end">
          <textarea
            value={replyText} onChange={(e) => setReplyText(e.target.value)}
            placeholder="Escreva uma resposta..."
            rows={1}
            className="flex-1 resize-none bg-card border border-foreground/10 rounded-md px-3 py-2 text-xs text-foreground placeholder:text-foreground/30 outline-none focus:border-[rgb(var(--lz-brand-rgb))]"
          />
        ...
        <div className="space-y-3">
          <div className="relative">
            <select value={categoryId} onChange={(e) => setCategoryId(e.target.value)}
              className="w-full appearance-none bg-card border border-foreground/10 rounded-md px-3 py-2 text-sm text-foreground outline-none focus:border-[rgb(var(--lz-brand-rgb))]">
          ...
          <input value={title} onChange={(e) => setTitle(e.target.value)} placeholder="Título"
            maxLength={200}
            className="w-full bg-card border border-foreground/10 rounded-md px-3 py-2 text-sm text-foreground placeholder:text-foreground/30 outline-none focus:border-[rgb(var(--lz-brand-rgb))]" />
          <textarea value={body} onChange={(e) => setBody(e.target.value)} placeholder="Escreva sua dúvida, ideia ou case..."
            rows={5} maxLength={8000}
            className="w-full resize-none bg-card border border-foreground/10 rounded-md px-3 py-2 text-sm text-foreground placeholder:text-foreground/30 outline-none focus:border-[rgb(var(--lz-brand-rgb))]" />
