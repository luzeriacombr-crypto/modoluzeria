  ...
  return (
    <div className="p-6 md:p-10 max-w-6xl mx-auto">
      <div className="flex items-end justify-between mb-8 flex-wrap gap-4">
        <div>
          <div className="flex items-center gap-2 text-foreground/40 text-xs uppercase tracking-wider mb-2">
            <Sparkles size={13} /> <span>Escala</span>
          </div>
          <h1 className="text-[32px] font-bold text-foreground leading-none tracking-tight">Rotina</h1>
          <p className="text-sm text-foreground/50 mt-2">Tabela semanal de tarefas recorrentes da equipe.</p>
        </div>
      </div>
      ...
      <div className="bg-card rounded-lg overflow-hidden">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-foreground/6">
                <th className="text-left text-[10px] uppercase font-bold tracking-wider text-foreground/40 px-4 py-3 w-[280px]">Tarefa</th>
                {CLEANING_DAYS.map((d) => (
                  <th key={d} className="text-left text-[10px] uppercase font-bold tracking-wider text-foreground/40 px-3 py-3">{d}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              ...
              {tasks.map((task, ti) => (
                <tr key={task.id} className="border-b border-foreground/4 last:border-b-0 group">
                    ...
                    {editingTaskId === task.id ? (
                      <div className="flex items-center gap-1.5">
                        <input
                          ...
                          className="flex-1 min-w-0 bg-background border border-foreground/10 rounded px-2 py-1 text-xs text-foreground outline-none focus:border-[rgb(var(--lz-brand-rgb))]"
                        />
                        <button onClick={saveEdit} className="p-1 rounded text-[var(--lz-accent-ink)] hover:bg-foreground/5 shrink-0"><Save size={13} /></button>
                        <button onClick={() => setEditingTaskId(null)} className="p-1 rounded text-foreground/40 hover:bg-foreground/5 shrink-0"><X size={13} /></button>
                      </div>
                    ) : (
                      <div className="flex items-center gap-1.5">
                        ...
                        {isMaster && (
                          <div className="flex items-center gap-0.5 opacity-40 group-hover:opacity-100 transition-opacity shrink-0">
                            ...
                            <button
                              ...
                              className="p-1 rounded text-foreground/40 hover:text-red-400 hover:bg-foreground/5"><Trash2 size={12} /></button>
                          </div>
                        )}
                      </div>
                    )}
                    ...
                    return (
                      <td key={wi} className="p-1.5 align-top">
                        ...
                        <button
                          ...
                          style={{
                            backgroundColor: isMine ? "rgba(var(--lz-brand-light-rgb),0.1)" : i ? "color-mix(in srgb, var(--foreground) 3%, transparent)" : "transparent",
                            border: isMine ? "1px solid rgb(var(--lz-brand-rgb))" : "1px solid color-mix(in srgb, var(--foreground) 5%, transparent)",
                        ...
                        {entry?.userId && (
                          <button
                            ...
                            style={{
                              backgroundColor:
                                status === "done" ? "var(--lz-accent-ink)"
                                : status === "missed" ? "#FF4444"
                                : "color-mix(in srgb, var(--foreground) 15%, transparent)",
                              color: status === "missed" ? "#FFFFFF" : "#0D0D0D",
                              cursor: canToggle ? "pointer" : "default",
                              border: status === "pending" ? "1px solid color-mix(in srgb, var(--foreground) 25%, transparent)" : "none",
                            }}
                          >
                  ...
                  <td colSpan={CLEANING_DAYS.length + 1} className="px-4 py-3">
                    <div className="flex items-center gap-2 max-w-sm">
                      <input
                        ...
                        className="flex-1 bg-background border border-foreground/10 rounded-md px-3 py-1.5 text-xs text-foreground outline-none focus:border-[rgb(var(--lz-brand-rgb))] placeholder:text-foreground/30"
                      />
                      <button
        ...
        <div className="border-t border-foreground/6 p-5">
          <div className="text-[10px] uppercase font-bold tracking-wider text-foreground/40 mb-2">Nota</div>
          {isAdmin ? (
            <div className="space-y-2">
              <textarea
                ...
                rows={3}
                className="w-full bg-background border border-foreground/10 rounded-md px-3 py-2 text-xs text-foreground/90 outline-none focus:border-[rgb(var(--lz-brand-rgb))] resize-none"
              />
              {noteDirty && (
                <button
            ...
            <p className="text-xs text-foreground/60 italic leading-relaxed">{noteDraft}</p>
          )}
        </div>
      </div>
