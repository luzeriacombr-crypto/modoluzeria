  ...
  return (
    <>
      <div data-tour="setup-checklist" className="rounded-2xl mb-6 p-5 md:p-6"
        style={{ background: "var(--card)", border: "1px solid rgba(var(--lz-brand-light-rgb),0.18)" }}>
        <div className="text-[10px] uppercase font-bold tracking-wider mb-3" style={{ color: "var(--lz-accent-ink)" }}>
          Primeiros passos
        </div>
        <div className="space-y-3">
          {steps.map((s) => (
            <div key={s.title} className="flex items-center gap-3 bg-foreground/[0.03] rounded-lg px-4 py-3">
              {s.done ? <CheckCircle2 size={18} style={{ color: "var(--lz-accent-ink)" }} className="shrink-0" /> : <Circle size={18} className="text-foreground/25 shrink-0" />}
              <div className="shrink-0" style={{ color: s.done ? "color-mix(in srgb, var(--foreground) 30%, transparent)" : "var(--lz-accent-ink)" }}>{s.icon}</div>
              <div className="flex-1 min-w-0">
                <div className={`text-sm font-semibold ${s.done ? "text-foreground/40 line-through" : "text-foreground"}`}>{s.title}</div>
                <div className="text-foreground/40 text-xs mt-0.5">{s.desc}</div>
              </div>
              {!s.done && s.action}
            </div>
          ))}
      ...
      {showImport && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/70 px-4" onClick={closeImport}>
          <div className="w-full max-w-md bg-card rounded-2xl p-7"
