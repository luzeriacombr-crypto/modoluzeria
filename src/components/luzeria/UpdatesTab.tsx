    ...
    <div className="max-w-2xl">
      ...
      {updates.length > 0 && (
        <div className="flex flex-wrap gap-1.5 mb-5">
          <button
            ...
            style={{
              backgroundColor: filter === null ? "rgb(var(--lz-brand-rgb))" : "color-mix(in srgb, var(--foreground) 6%, transparent)",
              color: filter === null ? "#0D0D0D" : "color-mix(in srgb, var(--foreground) 60%, transparent)",
            }}
          >
          ...
          {PLATFORM_UPDATE_CATEGORIES.filter((c) => categoryCounts.has(c)).map((c) => (
            <button
              ...
              className="rounded-full px-3 py-1.5 text-[11px] font-semibold transition-colors"
              style={{
                backgroundColor: filter === c ? "rgb(var(--lz-brand-rgb))" : "color-mix(in srgb, var(--foreground) 6%, transparent)",
                color: filter === c ? "#0D0D0D" : "color-mix(in srgb, var(--foreground) 60%, transparent)",
              }}
            >
      ...
      {filtered.length === 0 ? (
        ...
        <div className="space-y-4">
          {filtered.map((u) => (
            <div key={u.id} className="bg-card rounded-lg p-5 group relative">
              <div className="flex items-center gap-2 mb-1.5">
                <span
                  className="text-[10px] font-bold uppercase tracking-wide px-2 py-0.5 rounded-full"
                  style={{ background: "rgba(var(--lz-brand-light-rgb),0.14)", color: "var(--lz-accent-ink)" }}
                >
                  {u.category}
                </span>
              ...
              <h3 className="text-sm font-bold text-foreground mb-1.5">{u.title}</h3>
              <p className="text-sm text-foreground/60 leading-relaxed mb-3">{u.description}</p>
              {u.linkPath && (
                <button
