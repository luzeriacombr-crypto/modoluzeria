-- O preview público do cliente (/preview/$token) até agora só mostrava
-- posts/reels já em REVISAO_CLIENTE ou PRONTO_PARA_PUBLICAR — o cliente não
-- tinha visibilidade nenhuma do que estava em produção. Agora a página vai
-- mostrar TODOS os posts/reels do mês com um selo de etapa (estilo
-- Uber/iFood), então a função passa a filtrar por tipo em vez de status, e
-- expõe status/blocked_reason pro front decidir a etapa e o aviso.
--
-- Filtrar por ci.type também fecha uma brecha: antes, um `story` que
-- avançasse até REVISAO_CLIENTE/PRONTO_PARA_PUBLICAR (statusOptionsFor
-- permite isso) vazava pro preview público mesmo devendo ficar de fora.
