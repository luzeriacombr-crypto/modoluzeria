-- Direction for the "cronologica" order mode: recentes primeiro (desc) or
-- antigas primeiro (asc). Persisted per month like feed_order_mode, so the
-- public client-facing feed link matches whatever the agency picked.

ALTER TABLE public.months
  ADD COLUMN IF NOT EXISTS feed_order_direction text NOT NULL DEFAULT 'asc'
    CHECK (feed_order_direction IN ('asc', 'desc'));
