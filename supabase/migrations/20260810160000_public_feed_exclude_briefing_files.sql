-- Bug: o preview público do cliente (/preview/$token) estava mostrando as
-- imagens de referência do Briefing junto com a mídia real do post/reel.
-- Causa: get_public_feed() selecionava TODOS os item_files do item, sem
-- filtrar kind = 'media' — igual já é feito em todo o resto do código
-- (drive.functions.ts:358,418 usam .eq("kind", "media")). Arquivos de
-- referência têm kind = 'briefing' (migration 20260808120000) e nunca
-- deveriam aparecer pro cliente.
